...
function sheetFromRows(name, rows) {
  return { name: safeSheetName(name), rows };
}
...
function buildWorkbook(report) {
  ...
  sheets.push(sheetFromRows('Summary', [
    ['Metric', 'Value'],
    ['Generated At', report.generatedAt],
    ['Data Source', report.dataSource],
    ['Instrument', config.instrument || report.instrument || ''],
    ...
    ['Excel Generated By', 'Dependency-free XML exporter']
  ]));
  ...
  sheets.push(sheetFromRows('Trade Summary', tradeSummaryRows));
  ...
  sheets.push(sheetFromRows('Trades', [tradeHeaders, ...tradeRows]));

  const expiryRows = (report.expirySummaries || []).map((e) => [
    e.date, e.weekday, e.era, e.traded ? 'YES' : 'NO', e.numTrades || 0, e.bestMultiplier || '', e.skipReason || ''
  ]);
  sheets.push(sheetFromRows('All Expiries', [
    ['Date', 'Weekday', 'Era', 'Traded', 'Num Trades', 'Best Multiplier', 'Skip Reason'],
    ...expiryRows
  ]));
  ...
  sheets.push(sheetFromRows('By Year', [
  ...
  sheets.push(sheetFromRows('Breakdowns', [
    ['Breakdown', 'Value'],
    ...breakdownRows
  ]));
...
function writeBacktestExcel(inputPath = 'backtest-real-results.json', outDir = 'exports') {
  ...
  const instrument = report.config && report.config.instrument
    ...
    : '';
  const outName = `backtest-${instrument}${report.totalExpiries || 'results'}-expiries.xls`;
  const outPath = path.join(resolvedOutDir, outName);

  const wb = buildWorkbook(report);
  fs.writeFileSync(outPath, workbookToXml(wb), 'utf8');
  return outPath;
}

if (require.main === module) {
  try {
    const outPath = writeBacktestExcel(process.argv[2] || 'backtest-real-results.json', 'exports');
    console.log(outPath);
  } catch (error) {
