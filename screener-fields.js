...
const FIELDS = [
  ...
  { name: 'EMA 20', unit: 'INR', from: 'bars', computed: true,
    pick: (t) => t.movingAverages && t.movingAverages.ema20, alias: ['EMA20'] },
  { name: 'EMA 50', unit: 'INR', from: 'bars', computed: true,
    pick: (t) => t.movingAverages && t.movingAverages.ema50, alias: ['EMA50', 'EMA'],
    note: '`EMA` (bare) aliases EMA 50. There is no single EMA — name the period (EMA 20 / EMA 50).' },
  { name: 'VWAP 20', unit: 'INR', from: 'bars', computed: true,
    pick: (t) => t.movingAverages && t.movingAverages.vwap20, alias: ['VWAP'],
    note: 'Anchored VWAP over 20 DAILY sessions (close weighted by session volume). NOT the intraday chart VWAP — we have no intraday ticks. `VWAP` (bare) aliases the 20-session value.' },
  { name: 'VWAP 50', unit: 'INR', from: 'bars', computed: true,
    pick: (t) => t.movingAverages && t.movingAverages.vwap50 },
  { name: 'Price vs SMA 50', unit: 'percent', from: 'bars', computed: true,
    pick: (t) => t.movingAverages && t.movingAverages.vsSma50Pct, alias: ['vs SMA 50'],
    note: 'How far price is above/below the 50-day SMA, in %. Positive = above.' },
  { name: 'Price vs SMA 200', unit: 'percent', from: 'bars', computed: true,
    pick: (t) => t.movingAverages && t.movingAverages.vsSma200Pct, alias: ['vs SMA 200'] },
  { name: 'MACD', unit: 'INR', from: 'bars', computed: true,
    pick: (t) => t.oscillators && t.oscillators.macd },
  { name: 'MACD signal', unit: 'INR', from: 'bars', computed: true,
    pick: (t) => t.oscillators && t.oscillators.macdSignal },
  { name: 'MACD histogram', unit: 'INR', from: 'bars', computed: true,
    pick: (t) => t.oscillators && t.oscillators.macdHistogram, alias: ['MACD hist'] },
  { name: 'MACD cross', unit: 'text', type: 'text', from: 'bars', computed: true,
    pick: (t) => t.oscillators && t.oscillators.macdCross,
    note: 'BULLISH when MACD > signal, else BEARISH. Text — use with = / IN, not < / >.' },
  { name: 'RSI zone', unit: 'text', type: 'text', from: 'bars', computed: true,
    pick: (t) => t.oscillators && t.oscillators.rsiZone,
    note: 'OVERBOUGHT (RSI≥70) / OVERSOLD (≤30) / NEUTRAL. Text field.' },
  { name: 'Trend', unit: 'text', type: 'text', from: 'bars', computed: true,
    pick: (t) => t.trend && t.trend.label,
    note: 'UP / DOWN / SIDEWAYS / UNKNOWN from SMA alignment (price vs SMA50/200, SMA20 vs SMA50). Text field.' },
  ...
  { name: 'Change 3m', unit: 'percent', from: 'bars', computed: true,
    pick: (t) => t.performance && t.performance.m3 },
  { name: 'Change 6m', unit: 'percent', from: 'bars', computed: true,
    pick: (t) => t.performance && t.performance.m6 },
